...
class RandomForestGraphs(object):
  ...
  def _bag_features(self, tree_num, input_data):
    ...
    #print("index : ", self.params.bagged_features)
    return array_ops.concat([split_data[ind] for ind in self.params.bagged_features[tree_num]], 1)
  ...
  def training_graph(self,
    ...
    #Store the Tree Feature prepare to feed into FC layer
    total_trees_data = []
    ...
    for i in range(tree_start, tree_end):
      with ops.device(self.variables.device_dummies[i].device):
        ...
        if self.params.bagging_fraction < 1.0:
          ...
          total_gather_indices.append(gather_indices)
          total_trees_label.append(tree_labels)
        ...
        if self.params.bagged_features:
          ...
          #Bagging the feature.
          #The label always same for n trees, which means

          tree_data = self._bag_features(i, tree_data)

          #all the tress node data
          total_trees_data.append(tree_data)
    ...
    return tree_data, tree_labels, total_trees_data
  ...
  def inference_graph(self, input_data, **inference_args):
    ...
    with ops.device(self.variables.device_dummies[0].device):
      ...
      return average_values, tree_paths, regression_variance, probs
